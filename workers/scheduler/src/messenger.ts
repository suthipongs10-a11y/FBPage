...
export function buildMessengerDeps(prisma: PrismaClient, secret: string, env = process.env): MessengerDeps {
  ...
  return { prisma, secret, appId: env.META_APP_ID, testMode, allowAutomaticSend: testMode || env.MESSENGER_AUTO_SEND_ENABLED === 'true', aiTestBaseUrl: env.MESSENGER_AI_MOCK_BASE_URL || undefined, transport: new MetaMessenger({ version: env.META_GRAPH_API_VERSION, ...(testMode && { testMode, testBaseUrl: env.META_GRAPH_BASE_URL }) }) };
}
