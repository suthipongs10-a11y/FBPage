...
- ใช้ OpenAI key และโมเดลแยกจากงานคอนเทนต์ ไม่มีการดึงคีย์ทั่วไปหรือเปลี่ยนไปใช้ Gemini โดยอัตโนมัติ
...
2. ไป **แชทอัตโนมัติ** กรอก OpenAI API key ที่สร้างแยกสำหรับงานนี้ ระบุโมเดลที่บัญชีมีสิทธิ์ใช้และจำนวนเรียกสูงสุดต่อวัน แล้วบันทึก คีย์เข้ารหัสในฐานข้อมูลและแสดงเฉพาะตัวท้าย
...
เปลี่ยนคีย์หรือโมเดลจะปิดบอตของทุกเพจใน workspace นั้นชั่วคราว ต้องทดลองและเปิดเพจที่ต้องการใหม่ การลบคีย์แชทไม่ลบคีย์ AI สำหรับคอนเทนต์
...
- ความเร็วขึ้นกับโมเดล OpenAI เครือข่าย Meta และคิว มีการรวมข้อความที่มาถี่และยกเลิกคำตอบเก่าก่อนเริ่มส่ง กำหนดเวลารอ AI 25 วินาทีและรอ Meta 15 วินาที ไม่รับประกันว่าอินเทอร์เน็ตหรือผู้ให้บริการล่มแล้วยังส่งได้
...
- เพดานรายวันนับการเรียก AI ที่เริ่มแล้วรวมการทดลองและการเรียกที่ล้มเหลว แยกจากงบ AI คอนเทนต์ รีเซ็ต 00:00 UTC (07:00 เวลาไทย) ไม่ใช่การจำกัดเงิน USD และไม่ใช่ rate limit แยกจาก OpenAI organization โดยอัตโนมัติ ดูยอดเงินจริงใน OpenAI project
...
- Browser ทั้งระบบ **11 เคสผ่าน** รวมการกรอกคีย์แชท ทดลองคำตอบ เปิดเฉพาะเพจ รับ webhook ผ่านคิวจริง ส่งคำตอบจำลอง รับช่วงโดยแอดมิน และปิดเพจ
...
- การส่งจริงกับ OpenAI/Meta ยังเป็น **BLOCKED_BY_CREDENTIALS** และต้องยืนยันสิทธิ์แอป/เพจจริง ไม่มีการส่งข้อความถึงลูกค้าจริงระหว่างพัฒนา
...
The owner requested automatic AI replies without per-message approval, enabled only for selected Facebook Pages. The existing Facebook webhook accepts message events but its worker has no conversation processor. Existing AI credentials are unique per workspace/provider, so Messenger gets a separate encrypted OpenAI credential and model setting while reusing the shared AI adapter. Content generation continues to use its existing configuration.
