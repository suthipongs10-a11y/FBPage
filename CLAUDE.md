...
> Phase 10 Website Care W-1/W-2 (`packages/web-core`, `apps/api/src/web`, worker คิว `web-*`, หน้า `/web`) ตาม `AGENTS_WEB.md` — uptime/SSL/SEO/ลิงก์/PageSpeed + Search Console + SEO Analyst · กฎ: ตรวจสุภาพ (UA ระบุตัว, ≤ 1 คำขอ/วิ, เคารพ robots), กัน SSRF (ห้าม IP ภายในเว้น `WEB_ALLOW_PRIVATE_TARGETS` ใน test), ค่าที่อ่านไม่ได้ = null/SKIPPED, test ใช้ `startMockWeb()`
> Phase 11 Website Care W-3 บทความขึ้นเว็บ (`packages/web-core/wordpress.ts` + `web-publisher.ts`, `apps/api/src/web/web-content.service.ts`, คิว `web-publish`, หน้า `/web/content`) — ContentItem `platform=WEB` + `WebContentMetadata`, AI ร่างจากคลิป/โพสต์/แบรนด์ → Reviewer → อนุมัติ → WordPress REST (Application Password) กันซ้ำด้วย ExternalOperation
> Phase 11 W-4 อีเมลการตลาด (`packages/email-core`, `apps/api/src/email`, คิว `email-send`, หน้า `/email`) — Brevo/Resend ผ่าน HTTP ล้วน, รายชื่อ + consent (PDPA) + unsubscribe token, แคมเปญต้องอนุมัติก่อนส่ง, สถิติจาก webhook
> Phase 12 TikTok (`packages/tiktok-core`, `apps/api/src/tiktok`, หน้า `/tiktok`) ตาม `AGENTS_TIKTOK.md` — OAuth, ซิงก์คลิป+metric, Content Lab บน ContentItem (platform=TIKTOK), อนุมัติสองชั้น (APPROVED + `confirmedById`) และ `SOCIAL_PUBLISHING_ENABLED` (ค่าเริ่มต้นปิด) ถึงจะอัปโหลดได้ · media URL ต้องตรง `TIKTOK_VERIFIED_MEDIA_PREFIXES` (https + .mp4 + กัน SSRF)
> Phase 12 Messenger (`packages/messenger-core`, `apps/api/src/messenger`, คิว `messenger`, หน้า `/messenger`) ตาม `docs/MESSENGER.md` — รับ webhook เพจ, ตอบด้วย AI จากคลังความรู้แบรนด์, ค่าเริ่มต้นเก็บเป็น `DRAFT` + สลับเป็น `HUMAN` ต้องตั้ง `MESSENGER_AUTO_SEND_ENABLED=true` ถึงจะส่งเอง
> ห้องข่าว (`packages/web-core/src/news.ts`, `apps/api/src/news`, หน้า `/news`) ตาม `docs/NEWSROOM.md` — แหล่งข่าว RSS/คำค้น (Tavily, คีย์เข้ารหัส) → กันซ้ำ URL+หัวข้อ → AI research คัด → AI content เขียนใหม่ + ต่อท้ายที่มา + การ์ด `news` → รออนุมัติ · `POST content/:id/approve-schedule` อนุมัติ+ตั้งเวลาคลิกเดียว (บล็อกถ้ายังมี `[ต้องยืนยัน`) · เก็บแค่หัวข้อ/เกริ่น/ลิงก์ ห้ามใช้รูปสำนักข่าว · test ใช้ `/news/*` และ `/tavily/search` ใน `startMockWeb()`
> ทางเดิน AI กลางอยู่ที่ `packages/ai-core/task-runner.ts` (`runAiTask` / `runStructuredTask`) — `AiGatewayService` และ worker ใช้ตัวเดียวกัน ทุกโมดูล (แชทด้วย) จึงได้ บทบาท→โมเดล, BYOK, งบเดือน และ AiTaskLog เหมือนกัน
> คีย์ AI เก็บเป็น `AiConnection` (ตั้งชื่อเอง ใส่ชนิดเดียวกันได้หลายใบ เช่น compatible สามใบชี้ MiniMax/DeepSeek/Groq) — `AiRoleConfig` ชี้ `connectionId` ไม่ใช่ชนิด · แคตตาล็อกผู้ให้บริการอยู่ที่ `packages/ai-core/presets.ts` (เพิ่มเจ้าใหม่ = เพิ่มแถว ไม่ใช่เขียนอะแดปเตอร์) · เลือกคีย์+โมเดลเฉพาะครั้งได้ด้วย `meta.override` / body `modelOverride` (งบและ AiTaskLog ยังทำงานเหมือนเดิม บันทึก `connectionId` ที่ใช้จริงทุกครั้ง) · แผนต่อยอด (ค้นคว้า/กันโพสต์ซ้ำ/สร้างรูป-คลิป) อยู่ใน `docs/CONTENT_ENGINE_PLAN.md`
> WEB_PUBLISH_ENABLED=true WEB_WP_ALLOW_INSECURE=true WEB_MOCK_BASE_URL=... pnpm dev:api  · EMAIL_SEND_ENABLED=true EMAIL_MOCK_BASE_URL=...  (ค่าเริ่มต้นปิดทั้งคู่ = ร่าง/อนุมัติได้ แต่ยังไม่ยิงขึ้นเว็บ/ไม่ส่งอีเมล)
> กฎ W-3/W-4 เพิ่มเติม: ห้ามแก้เว็บลูกค้าโดยไม่ผ่านอนุมัติ (โพสต์ได้เฉพาะ APPROVED) · รหัสผ่าน WordPress/API key ผู้ให้บริการอีเมลเข้ารหัสด้วย `common/crypto` ห้ามอยู่ใน select/response/audit · เชื่อม WordPress ต้อง https (test เท่านั้นที่ยอม http) · ส่งอีเมลได้เฉพาะผู้รับที่ยินยอมและยังไม่ยกเลิก ทุกฉบับมีลิงก์ยกเลิกรับ · สถิติเปิด/คลิกที่ไม่มี webhook = null ห้ามเป็น 0 · ห้ามใช้ SMTP แจ้งเตือนภายในส่งการตลาดจำนวนมาก · test ใช้ `startMockWeb()` (มี WordPress จำลอง) และ `startMockEmailProvider()`
...
> กฎที่ต้องรักษาในระบบใหม่: token/API key เข้ารหัสด้วย `common/crypto.ts` เท่านั้น ห้ามอยู่ใน select/response/audit · metric ที่อ่านไม่ได้ = `null` ห้ามแปลงเป็น 0 · test ห้ามยิงเพจจริง/AI จริง ใช้ `startMockGraph()` / `startMockAi()` · โค้ดเรียก AI ต้องผ่าน `AiGatewayService` (ใน API) หรือ `runAiTask`/`runStructuredTask` ของ `@fbpm/ai-core` (ใน worker/package) เท่านั้น ห้าม import SDK ผู้ให้บริการ และห้ามเก็บ API key ของ AI แยกรายโมดูล
>
> ---
