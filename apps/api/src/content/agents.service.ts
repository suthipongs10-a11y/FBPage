...
export class ContentAgentsService {
  ...
  async generate(workspaceId: string, userId: string, pageId: string, dto: GenerateDto, requestId: string, contentId?: string) {
    ...
    const out = await this.ai.structured({ workspaceId, userId, taskType: 'content.generate', role: 'content', requestId, promptVersion: CONTENT_PROMPT_VERSION, override: dto?.modelOverride ?? null, resourceType: existing ? 'contentItem' : 'facebookPage', resourceId: existing?.id ?? pageId }, {
