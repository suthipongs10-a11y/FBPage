...
1. **หน้าจอฝั่งปฏิบัติการยังอ่านข้อมูลตัวอย่าง** — ⚠️ ตัวกั้นที่ใหญ่ที่สุดตอนนี้
   `/`, `/inbox`, `/calendar`, `/ops`, `/pages` โหลดจาก `apps/web/src/lib/demo-workspace.ts`
   ซึ่งเป็นข้อมูลสมมติ ต้องเขียนตัว implement `WorkspaceSource` ที่อ่านจาก Prisma
   มาเสียบแทน (ชนิดข้อมูลกำหนดไว้ครบแล้วที่ `lib/workspace.ts` — หน้าจอไม่ต้องแก้)

   ระหว่างนี้ทุกหน้าที่ใช้ข้อมูลสมมติติดป้าย `<DemoBanner />` ไว้บนสุดแล้ว
   **ห้ามเอาป้ายออกจนกว่าหน้านั้นจะต่อของจริง** — เคยทำให้คนที่เพิ่งติดตั้งเสร็จ
   เห็น "มี 3 เรื่องที่พังอยู่ตอนนี้" ทั้งที่ยังไม่ได้เชื่อมเพจสักเพจ

   หน้า `/settings` เป็นหน้าเดียวที่อ่านของจริงอยู่แล้ว (จึงไม่มีป้าย)
