...
อัปเดตล่าสุด: หลังจบ M-J (apps/webhook + apps/worker + packages/queue + packages/store)
...
| `apps/worker` | cron ทุกนาที: หาโพสต์ที่ถึงเวลา → ยิงขึ้นเพจ → บันทึกผล |
...
1. **`InboxStore` บน Prisma** — ⚠️ ตอนนี้ `apps/webhook` ยัด event เข้าคิวได้แล้ว
   แต่ยังไม่มีใครหยิบไปทำ ถ้าเปิดรับ webhook จริงงานจะกองใน Redis ไปเรื่อยๆ
   ต้องเขียนตัวนี้ก่อนถึงจะเปิดรับได้
2. **หน้าเชื่อมเพจ (OAuth callback)** ใน `apps/web` — ยังไม่มีวิธีใส่ token เข้าระบบ
3. **ปลายทางแจ้งเตือนจริง (LINE)** — ตรวจเจอปัญหาแล้วแต่ยังไม่ได้ส่งไปไหน
